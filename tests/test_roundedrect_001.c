...
// Test: nvgRoundedRect with parameters: {'x': 150, 'y': 100, 'w': 80, 'h': 80, 'r': 20}

int main(void)
{
	printf("=== Testing nvgRoundedRect variant 1 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgRoundedRect Test");
	...
	nvgBeginPath(vg);
	nvgRoundedRect(vg, 150, 100, 80, 80, 20);
	...
	nvgText(vg, 10, 20, "nvgRoundedRect - variant 1", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_roundedrect_001.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgRoundedRect variant 1\n");
	return 0;
}
