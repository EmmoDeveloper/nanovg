...
// Test: nvgArc with parameters: {'cx': 200, 'cy': 100, 'r': 30, 'a0': 0, 'a1': 6.28, 'dir': 'NVG_CCW'}

int main(void)
{
	printf("=== Testing nvgArc variant 1 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgArc Test");
	...
	nvgBeginPath(vg);
	nvgArc(vg, 200, 100, 30, 0, 6.28f, NVG_CCW);
	...
	nvgText(vg, 10, 20, "nvgArc - variant 1", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_arc_001.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgArc variant 1\n");
	return 0;
}
