...
// Test: nvgRoundedRectVarying with parameters: {'x': 10, 'y': 200, 'w': 100, 'h': 50, 'radTopLeft': 5, 'radTopRight': 10, 'radBottomRight': 15, 'radBottomLeft': 20}

int main(void)
{
	printf("=== Testing nvgRoundedRectVarying variant 0 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgRoundedRectVarying Test");
	...
	nvgBeginPath(vg);
	nvgRoundedRectVarying(vg, 10, 200, 100, 50, 5, 10, 15, 20);
	...
	nvgText(vg, 10, 20, "nvgRoundedRectVarying - variant 0", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_roundedrectvarying_000.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgRoundedRectVarying variant 0\n");
	return 0;
}
