...
// Test: nvgTextDirection with parameters: {'direction': 'NVG_TEXT_DIR_RTL'}

int main(void)
{
	printf("=== Testing nvgTextDirection variant 2 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgTextDirection Test");
	...
	nvgTextDirection(vg, NVG_TEXT_DIR_RTL);
	nvgFontFace(vg, "sans");
	...
	nvgText(vg, 10, 20, "nvgTextDirection - variant 2", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_textdirection_002.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgTextDirection variant 2\n");
	return 0;
}
