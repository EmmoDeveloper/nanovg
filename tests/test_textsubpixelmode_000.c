...
// Test: nvgTextSubpixelMode with parameters: {'mode': 0}

int main(void)
{
	printf("=== Testing nvgTextSubpixelMode variant 0 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgTextSubpixelMode Test");
	...
	nvgText(vg, 10, 20, "nvgTextSubpixelMode - variant 0", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_textsubpixelmode_000.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgTextSubpixelMode variant 0\n");
	return 0;
}
