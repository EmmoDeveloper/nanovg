...
// Test: nvgBeginFrame with parameters: {'width': 1920, 'height': 1080, 'ratio': 2.0}

int main(void)
{
	printf("=== Testing nvgBeginFrame variant 1 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgBeginFrame Test");
	...
	nvgBeginFrame(vg, 1920, 1080, 2.0f);
	nvgBeginPath(vg);
	...
	nvgText(vg, 10, 20, "nvgBeginFrame - variant 1", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_beginframe_001.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgBeginFrame variant 1\n");
	return 0;
}
