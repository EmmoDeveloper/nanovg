...
// Test: nvgTransform with parameters: {'a': 1.5, 'b': 0.0, 'c': 0.0, 'd': 1.5, 'e': 100.0, 'f': 50.0}

int main(void)
{
	printf("=== Testing nvgTransform variant 1 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgTransform Test");
	...
	nvgTransform(vg, 1.5f, 0.0f, 0.0f, 1.5f, 100.0f, 50.0f);
	nvgBeginPath(vg);
	...
	nvgText(vg, 10, 20, "nvgTransform - variant 1", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_transform_001.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgTransform variant 1\n");
	return 0;
}
