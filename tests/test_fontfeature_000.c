...
// Test: nvgFontFeature with parameters: {'tag': 'NVG_FEATURE_LIGA', 'enabled': 1}

int main(void)
{
	printf("=== Testing nvgFontFeature variant 0 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgFontFeature Test");
	...
	nvgText(vg, 100, 300, "fi fl ff ffi ffl", NULL);

	// Draw label
	nvgFontSize(vg, 14.0f);
	nvgFontFace(vg, "sans");
	nvgFillColor(vg, nvgRGBA(255, 255, 255, 255));
	nvgText(vg, 10, 20, "nvgFontFeature - variant 0", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_fontfeature_000.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgFontFeature variant 0\n");
	return 0;
}
