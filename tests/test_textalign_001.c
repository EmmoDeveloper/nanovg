...
// Test: nvgTextAlign with parameters: {'align': 'NVG_ALIGN_CENTER | NVG_ALIGN_MIDDLE'}

int main(void)
{
	printf("=== Testing nvgTextAlign variant 1 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgTextAlign Test");
	...
	nvgText(vg, 10, 20, "nvgTextAlign - variant 1", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_textalign_001.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgTextAlign variant 1\n");
	return 0;
}
