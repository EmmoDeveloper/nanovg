...
// Test: nvgFontFeature with parameters: {'tag': 'NVG_FEATURE_CALT', 'enabled': 1}

int main(void)
{
	printf("=== Testing nvgFontFeature variant 1 ===\n");
	...
	nvgText(vg, 100, 300, "The quick brown", NULL);

	// Draw label
	nvgFontSize(vg, 14.0f);
	nvgFontFace(vg, "sans");
	nvgFillColor(vg, nvgRGBA(255, 255, 255, 255));
	nvgText(vg, 10, 20, "nvgFontFeature - variant 1", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_fontfeature_001.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgFontFeature variant 1\n");
	return 0;
}
