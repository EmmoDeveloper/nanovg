...
// Test: nvgGlobalCompositeOperation with parameters: {'op': 'NVG_DESTINATION_OVER'}

int main(void)
{
	printf("=== Testing nvgGlobalCompositeOperation variant 4 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgGlobalCompositeOperation Test");
	...
	nvgText(vg, 10, 20, "nvgGlobalCompositeOperation - variant 4", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_globalcompositeoperation_004.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgGlobalCompositeOperation variant 4\n");
	return 0;
}
