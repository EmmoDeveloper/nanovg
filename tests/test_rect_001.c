...
// Test: nvgRect with parameters: {'x': 150, 'y': 10, 'w': 80, 'h': 80}

int main(void)
{
	printf("=== Testing nvgRect variant 1 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgRect Test");
	...
	nvgText(vg, 10, 20, "nvgRect - variant 1", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_rect_001.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgRect variant 1\n");
	return 0;
}
