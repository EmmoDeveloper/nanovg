...
// Test: nvgIntersectScissor with parameters: {'x': 25, 'y': 25, 'w': 150, 'h': 150}

int main(void)
{
	printf("=== Testing nvgIntersectScissor variant 0 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgIntersectScissor Test");
	...
	nvgIntersectScissor(vg, 25, 25, 150, 150);
	nvgBeginPath(vg);
	...
	nvgText(vg, 10, 20, "nvgIntersectScissor - variant 0", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_intersectscissor_000.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgIntersectScissor variant 0\n");
	return 0;
}
