...
// Test: nvgRestore with parameters: {}

int main(void)
{
	printf("=== Testing nvgRestore variant 0 ===\n");

	WindowVulkanContext* winCtx = window_create_context(800, 600, "nvgRestore Test");
	...
	nvgText(vg, 10, 20, "nvgRestore - variant 0", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/test_restore_000.ppm");

	nvgDeleteVk(vg);
	window_destroy_context(winCtx);

	printf("Test PASSED: nvgRestore variant 0\n");
	return 0;
}
