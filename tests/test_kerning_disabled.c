...
int main(void) {
	printf("=== Kerning DISABLED Test ===\n\n");

	WindowVulkanContext* winCtx = window_create_context(800, 400, "Kerning Disabled");
	if (!winCtx) return 1;
	...
	printf("Text width WITHOUT kerning: %.2f pixels\n\n", width);
	...
	nvgText(vg, 50, 50, "WITHOUT Kerning", NULL);
	...
	window_save_screenshot(winCtx, imageIndex, "screendumps/kerning_off.ppm");
	printf("Saved: kerning_off.ppm\n");
