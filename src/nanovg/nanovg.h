...
// the blend func is based on standard graphics API conventions (see Vulkan blend modes for more info).
...
// Sets the subpixel rendering mode. 0=none, 1=RGB, 2=BGR, 3=VRGB, 4=VBGR.
...
struct NVGparams {
	...
	int colorSpace;  // Target color space (VkColorSpaceKHR) for text rendering (0 = default sRGB)
	int (*renderCreate)(void* uptr);
	int (*renderCreateTexture)(void* uptr, int type, int w, int h, int imageFlags, const unsigned char* data);
	int (*renderDeleteTexture)(void* uptr, int image);
	int (*renderUpdateTexture)(void* uptr, int image, int x, int y, int w, int h, const unsigned char* data);
	int (*renderGetTextureSize)(void* uptr, int image, int* w, int* h);
	int (*renderCopyTexture)(void* uptr, int srcImage, int dstImage, int srcX, int srcY, int dstX, int dstY, int w, int h);
